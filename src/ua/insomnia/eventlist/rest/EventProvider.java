...
public class EventProvider extends ContentProvider {
	...
	// "projection" map of all the events table columns
	private static HashMap<String, String> map;
	// URI matcher ID for the main events URI pattern
	private static final int MATCHER_EVENTS = 1;
	// URI matcher ID for the single event ID pattern
	private static final int MATCHER_EVENT_ID = 2;
	// URI matcher for validating URIs
	private static final UriMatcher uriMatcher;
	// Handle to our DbHelper.
	private DbHelper dbHelper;

	/**
	 * The MIME type of a events
	 */
	public static final String EVENT_CONTENT_TYPE = "vnd.android.cursor.dir/vnd.ua.insomnia.kpievent.events";
	/**
	 * The MIME type of a single event from the events table
	 */
	private static final String EVENT_CONTENT_ITEM_TYPE = "vnd.android.cursor.item/vnd.ua.insomnia.kpievent.events";
	...
	static {
		...
		uriMatcher.addURI(EventProviderContract.AUTHORITY,
				EventTable.TABLE_NAME, MATCHER_EVENTS);
		// Add a pattern to route URIs terminated with comic IDs
		uriMatcher.addURI(EventProviderContract.AUTHORITY,
				EventTable.TABLE_NAME + "/#", MATCHER_EVENT_ID);
		...
		for (String column : EventTable.ALL_COLUMNS) {
	...
	public int delete(Uri uri, String whereClause, String[] whereValues) {
		...
		try {
			...
			case MATCHER_EVENTS:
				// Delete all the rage comics matching the where column/value
				// pairs
				deletedRowsCount = db.delete(EventTable.TABLE_NAME,
						whereClause, whereValues);
				break;
			case MATCHER_EVENT_ID:
				// Delete the comic with the given ID
				String eventId = uri.getPathSegments().get(
						EventTable.EVENT_ID_PATH_POSITION);
				finalWhere = EventTable._ID + " = " + eventId;
				if (whereClause != null) {
					finalWhere = finalWhere + " AND " + whereClause;
				}
				// Perform the delete.
				deletedRowsCount = db.delete(EventTable.TABLE_NAME, finalWhere,
						whereValues);
				break;
	...
	public Uri insert(Uri uri, ContentValues initialValues) {
		...
		if (uriMatcher.match(uri) != MATCHER_EVENTS) {
			throw new IllegalArgumentException("Unknown URI " + uri);
		}
		...
		if (initialValues != null) {
			...
			throw new SQLException(
					"ContentValues arg for .insert() is null, cannot insert row.");
		}
		long newRowId = this.dbHelper.getWritableDatabase().insert(
				EventTable.TABLE_NAME, null, values);
		if (newRowId > 0) { 
			// if rowID is -1, it means the insert failed
			// Build a new Events URI with the new event's ID appended to it.
			Uri eventUri = ContentUris.withAppendedId(
					EventTable.CONTENT_ID_URI_BASE, newRowId);
	...
	public Cursor query(Uri uri, String[] selectedColumns, String whereClause,
		...
		case MATCHER_EVENTS:
			...
			break;
		// asking for a single comic - use the rage comics projection, but add a
		// where clause to only return the one
		// comic
		case MATCHER_EVENT_ID:
			qb.setProjectionMap(map);
			...
			String id = uri.getPathSegments().get(
					EventTable.EVENT_ID_PATH_POSITION);
			qb.appendWhere(EventTable._ID + "=" + id);
			break;
		default:
		...
		Cursor cursor = qb.query(db, selectedColumns, whereClause, whereValues,
				null, null, sortOrder);
	...
	public int update(Uri uri, ContentValues updateValues, String whereClause,
		...
		try {
			...
			case MATCHER_EVENTS:
				...
				updatedRowsCount = db.update(EventTable.TABLE_NAME,
						updateValues, whereClause, whereValues);
				break;
			case MATCHER_EVENT_ID:
				String id = uri.getPathSegments().get(
						EventTable.EVENT_ID_PATH_POSITION);
				finalWhere = EventTable._ID + " = " + id;
				// if we were passed a 'where' arg, add that to our 'finalWhere'
				if (whereClause != null) {
					finalWhere = finalWhere + " AND " + whereClause;
				}
				updatedRowsCount = db.update(EventTable.TABLE_NAME,
						updateValues, finalWhere, whereValues);
				break;
			default:
	...
	public int bulkInsert(Uri uri, ContentValues[] values) {
		...
		try {
			...
			Uri nodeUri = ContentUris.withAppendedId(
					EventTable.CONTENT_ID_URI_BASE, newRowId);
			// Notify observers that our data changed.
	...
	private long insert(Uri uri, ContentValues initialValues,
		...
		long newRowId = writableDb.insert(EventTable.TABLE_NAME, null, values);
